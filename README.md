...
Set webhook with params:

```bash
curl --location 'https://api.telegram.org/bot{bot_token}/setWebhook' \
--header 'Content-Type: application/json' \
--data '{
    "url": "https://api.techinterview.space/api/integrations/telegram/webhook",
    "secret_token": "{token}",
    "allowed_updates": [
      "channel_post",
      "edited_channel_post",
...
```bash
curl --location --request GET 'https://api.telegram.org/bot{bot_token}/getWebhookInfo' \
--header 'Content-Type: application/json'
```